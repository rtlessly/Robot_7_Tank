...
final static int SERIAL_PORT_BAUD_RATE = 115200; //57600;
...
boolean readToken(Serial serial, String token) 
{
  ...
  if (serial.available() < token.length()) return false;
  
  // Check if incoming bytes match token
  for (int i = 0; i < token.length(); i++) 
  {
    if (serial.read() != token.charAt(i)) return false;
  }
...
void draw() 
{
  ...
  if (serial.available() > 0)
  {
    ...
    {
      // ignore exception and continue - assumes any exception is a formatting exception
    }
  }
  else 
  {
